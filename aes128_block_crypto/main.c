...
// 将输入文本转化成 需要的矩阵
void text_to_matrix(const uint8_t text[16], uint8_t target_matrix[4][4]) {
    for (int k = 0; k < 16; k++) {
        int i = k % 4;
        int j = k / 4;
...
// 对块进行加密
void block_encrypt(uint8_t message_text[16], uint8_t key_text[16], uint8_t *ret) {
    // 秘密扩展
    key_expansion(key_text);
    //明文矩阵
    uint8_t state_matrix[4][4];
    text_to_matrix(message_text, state_matrix);
//    puts("plaintext matrix:");
...
void block_decrypt(uint8_t message_text[16], uint8_t key_text[16], uint8_t *ret) {
    // 秘密扩展
    key_expansion(key_text);
    // 秘文矩阵
    uint8_t state_matrix[4][4];
...
int main() {
    uint8_t key_text[16] = {0x3C, 0xA1, 0x0B, 0x21, 0x57, 0xF0, 0x19, 0x16, 0x90, 0x2E, 0x13, 0x80, 0xAC, 0xC1, 0x07,
                            0xBD};
/*    uint8_t plaintext_text[16] = "abcdefghijklmnop";
    uint8_t ret[16];
    block_encrypt(plaintext_text, key_text, ret);
    for (int i = 0; i < 16; ++i) {
        printf("0x%02X,", ret[i]);
    }
    puts("");*/
    uint8_t cryher[16] = {0x28, 0x63, 0xF7, 0xA2, 0x98, 0x18, 0xBC, 0xA5, 0xF4, 0x3A, 0xDF, 0x53, 0x70, 0x20, 0xA0,
                          0x8C};
    uint8_t ret2[16];
    block_decrypt(cryher, key_text, ret2);
    for (int i = 0; i < 16; ++i) {
        printf("%c", ret2[i]);
    }

/*    uint8_t plaintext_text[16] = {0x12, 'a', 'a', 'a', 0xAB, 'a', 'x', 'a', 'c', 'a', 'a', 'a', 'a', 'a', 'a', 'a'};
    uint8_t plaintext_matrix[4][4];
    text_to_matrix(plaintext_text, plaintext_matrix);
    puts("origin matrix:");
    print_matrix(plaintext_matrix);
    sbox_replace_matrix(plaintext_matrix);
    puts("sbox replace:");
    print_matrix(plaintext_matrix);
    invsbox_replace_matrix(plaintext_matrix);
    puts("inv_sbox replaceL");
    print_matrix(plaintext_matrix);*/

/*    uint8_t test_text[16] = "aaaabbbbccccdddd";
    uint8_t target[4][4];
    text_to_matrix(test_text, target);
    puts("origin matrix:");
    print_matrix(target);
    shift_row(target);
    puts("shift matrix:");
    print_matrix(target);
    inv_shift_row(target);
    puts("inv_shift matrix:");
    print_matrix(target);*/

/*    uint8_t test_matrix[4][4] = {
            {0xC9, 0xE5, 0xFD, 0x2B},
            {0x7A, 0xF2, 0x78, 0x6E},
            {0x63, 0x9C, 0x26, 0x67},
            {0xB0, 0xA7, 0x82, 0xE5}
    ...
    mix_column(test_matrix);
    puts("mix matrix:");
    print_matrix(test_matrix);
    inv_mix_column(test_matrix);
    puts("inv mix matrix:");
    print_matrix(test_matrix);*/
...
/*    uint8_t plaintext_text[16] = "abcdefghijklmnop";
    uint8_t key_text[16] = {0x3C, 0xA1, 0x0B, 0x21, 0x57, 0xF0, 0x19, 0x16, 0x90, 0x2E, 0x13, 0x80, 0xAC, 0xC1, 0x07,
                            0xBD};
    block_encrypt(plaintext_text, key_text);*/

/*    uint8_t plaintext_text[16] = "abcdefghijklmnop";
    uint8_t matrix[4][4];
    text_to_matrix(plaintext_text, matrix);
    print_matrix(matrix);
    uint8_t text[16];
    matrix_to_text(text, matrix);
//    printf("text:\n%s\n", text);
    for (int i = 0; i < 16; ++i) {
        printf("%02X ",text[i]);
    }
